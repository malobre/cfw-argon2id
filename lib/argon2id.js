...
const isLittleEndian =
  new Uint8Array(new Uint16Array([0xabcd]).buffer)[0] === 0xcd;
...
function LE64(buf, n, i) {
  if (n > Number.MAX_SAFE_INTEGER)
    throw new Error("LE64: large numbers unsupported");
  ...
  for (let offset = i; offset < i + 7; offset++) {
    // last byte can be ignored as it would overflow MAX_SAFE_INTEGER
...
function* makePRNG(
  ...
  for (let i = 1; i <= segmentLength; i++) {
    ...
    const g2 = G2(
      wasmContext,
      wasmContext.refs.ZERO1024,
      wasmContext.refs.prngTmp,
      wasmContext.refs.prngR,
    );
...
function validateParams({
  ...
  if (type !== TYPE || version !== VERSION)
    throw new Error("Unsupported type or version");
...
export default function argon2id(params, { memory, instance: wasmInstance }) {
  ...
  if (memory.buffer.byteLength < requiredMemory) {
    const missing = Math.ceil(
      (requiredMemory - memory.buffer.byteLength) / WASM_PAGE_SIZE,
    );
  ...
  const blockMemory = new Uint8Array(
    memory.buffer,
    offset,
    ctx.memorySize * ARGON2_BLOCK_SIZE,
  );
  ...
  const initBlock = (i, j) => {
    B[i][j] = blockMemory.subarray(
      i * q * 1024 + j * 1024,
      i * q * 1024 + j * 1024 + ARGON2_BLOCK_SIZE,
    );
    return B[i][j];
  };
