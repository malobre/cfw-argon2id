...
class Blake2b {
  constructor(outlen, key, salt, personal) {
    ...
    const params32 = new Uint32Array(
      params.buffer,
      params.byteOffset,
      params.length / Uint32Array.BYTES_PER_ELEMENT,
    );
...
export default function createHash(outlen, key, salt, personal) {
  if (outlen > OUTBYTES_MAX)
    throw new Error(
      `outlen must be at most ${OUTBYTES_MAX} (given: ${outlen})`,
    );
  if (key) {
    if (!(key instanceof Uint8Array))
      throw new Error("key must be Uint8Array or Buffer");
    if (key.length > KEYBYTES_MAX)
      throw new Error(
        `key size must be at most ${KEYBYTES_MAX} (given: ${key.length})`,
      );
  }
  if (salt) {
    if (!(salt instanceof Uint8Array))
      throw new Error("salt must be Uint8Array or Buffer");
    if (salt.length !== SALTBYTES)
      throw new Error(
        `salt must be exactly ${SALTBYTES} (given: ${salt.length}`,
      );
  }
  if (personal) {
    if (!(personal instanceof Uint8Array))
      throw new Error("personal must be Uint8Array or Buffer");
    if (personal.length !== PERSONALBYTES)
      throw new Error(
        `salt must be exactly ${PERSONALBYTES} (given: ${personal.length}`,
      );
  }
